...
function RoyalConversionRate() {
  const { t } = useTranslation('report');
  const { t: tCommon } = useTranslation('common');
  const { data: profile } = useProfile();

  const { data = [] } = useSWR('/royal-conversion-rate', () => ReportApis.getLoyalConversionRate());
  ...
  return (
    <PrivateLayout title={tCommon('loyalConversionRate')}>
      <ConversionRateChart
        charts={[
          {
            type: 'BAR',
      ...
      <h5 className='text-gray-600 mt-[60px] font-bold'>
        {t('measuresThatContributedToLoyalConversion')}
      </h5>
...
export default RoyalConversionRate;
