...
export const ImageUploader = ({ originalUrl, className }: Props) => {
  ...
  const renderOriginalUrl = () => (
    <div className='w-[120px] h-[120px] rounded flex justify-center items-center bg-dark-gray'>
      {originalUrl ? (
        <img src={originalUrl} alt='Logo' className={classNames('object-cover rounded w-full h-full', className)} />
      ) : (
  ...
  const renderUploadingImage = () => (
    <div className={classNames('w-[120px] h-[120px]', uploading ? 'opacity-50' : 'opacity-100')}>
      <img src={image?.preview} alt='Logo' className={classNames('object-cover rounded w-full h-full', className)} />
    </div>
  );

  return (
    <div>
      <Dropzone onDrop={handleDrop} accept='image/*' multiple={false} minSize={1024} maxSize={3072000}>
        {({ getRootProps, getInputProps }) => (
          <div {...getRootProps({ className: 'dropzone' })} className='flex items-center w-full text-center'>
            <input {...getInputProps()} />
