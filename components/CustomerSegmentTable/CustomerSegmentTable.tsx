...
const dashboardCardHeaderClasses = (status: string, index: number, length: number) =>
  classNames({
      ...
      'flex flex-col text-white relative min-w-44 w-full h-20 flex items-center justify-end py-3 border-white': true
  });
...
const getSecondDataPoint = (dataSet: { target: string; other: string; royalSleep: string; total: string; }) => {
  switch (dataSet.target) {
    case 'f0': 
